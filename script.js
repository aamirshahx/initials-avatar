...
function createAvatar(el) {
    ...
    if (name) {
		...
		const bgcolor = colors[parseInt(name.toLocaleLowerCase().split('')
            .map(c => c.charCodeAt(0).toString()).join('')) % colors.length];
        el.setAttribute("style", "background-color:" + bgcolor);
    }
}
